...
attribute vec3 aParam;    // x = opacity, y = drag, z = gravity
...
void main() {
  ...
  if (t < 0.0 || n >= 1.0) {
    ...
    gl_PointSize = 0.0;
    vAlpha = 0.0; vColor = vec3(0.0); vSeed = 0.0;
    return;
  }
  ...
  gl_PointSize = clamp(size * uPix * 300.0 / max(1.0, -mv.z), 1.0, 900.0);
...
export class ParticlePool {
  ...
  spawn(pos, vel, {
    life = 1.5, size0 = 1, size1 = 4, color = 0xffffff, opacity = 0.6,
    drag = 0.8, gravity = 0, delay = 0,
  } = {}) {
    const i = this._cursor;
    this._cursor = (this._cursor + 1) % this.capacity;
    const p = this.aPos.array, v = this.aVel.array, s = this.aSize.array, c = this.aColor.array, pr = this.aParam.array;
    ...
    pr[i * 3] = opacity; pr[i * 3 + 1] = drag; pr[i * 3 + 2] = gravity;
...
class WeaponFx {
  ...
  /** Persistent topside fire/smoke column for a hull that is burning. */
  hullFire(pos, intensity = 0.5) {
    // Called every sim frame — thin the spawn rate so the pool stays readable.
    if (Math.random() > 0.28 + intensity * 0.25) return;
    const by = (pos.y || 0) + 7 + intensity * 5;
    const n = intensity > 0.4 ? 3 : 2;
    for (let i = 0; i < n; i++) {
      _v2.set(
      ...
      _v.set((Math.random() - 0.5) * 6, 14 + Math.random() * 22, (Math.random() - 0.5) * 6);
      this.smoke.spawn(_v2, _v, {
        life: 2.2 + Math.random() * 2.0,
        size0: 3.5 + intensity * 4,
        size1: 16 + intensity * 22,
        color: 0x2e2c28,
        opacity: 0.42 + intensity * 0.35,
        drag: 1.0,
        gravity: -2.8,
      });
    }
    if (Math.random() < 0.65 + intensity * 0.3) {
      ...
      _v.set((Math.random() - 0.5) * 4, 8 + Math.random() * 14, (Math.random() - 0.5) * 4);
      this.fire.spawn(_v2, _v, {
        life: 0.28 + Math.random() * 0.28,
        size0: 5.5 + intensity * 7,
        size1: 1.6,
        color: Math.random() < 0.45 ? 0xffcf6a : 0xff7a2e,
        opacity: 0.95,
        drag: 2.0,
        gravity: -1.4,
      });
